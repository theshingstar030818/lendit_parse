...
import { NavParams ,ViewController, Content, InfiniteScroll, AlertController  } from 'ionic-angular';
...
export class CalendarModal {
    ...
    _disableDateSelected : boolean = false;
    ...
    init(){
        ...
        this.options = {
            ...
            disableDays:params.get('disableDays'),
    ...
    onChange(data: any) {
        ...
        this._disableDateSelected = this.disabledDaysInSelection();
    }
    ...
    onCancel() {
        ...
        if(!this.disabledDaysInSelection()){
            this.viewCtrl.dismiss(this.dayTemp);
        }
    }

    disabledDaysInSelection() : boolean {
        let retVal:boolean = false;
        if(this._d.isRadio){
            if(!!(this.dayTemp[0] && this.dayTemp[0].time)){
                if(this.options.disableDays.indexOf(this.dayTemp[0].time) !== -1) {
                    return true;
                }
            }
        }else {
            if(!!(this.dayTemp[0] && this.dayTemp[1]) && !!( this.dayTemp[0].time && this.dayTemp[1].time)){
                for(let d=0; d< this.options.disableDays.length;d++){
                    if(this.options.disableDays[d] >= this.dayTemp[0].time && this.options.disableDays[d] <= this.dayTemp[1].time  ){
                        retVal = true;
                        break;
                    }
                }
        ...
        if(retVal){
            this.showDisabledDateSelectedAlert();
        }
        return retVal;
    }

    showDisabledDateSelectedAlert() {
        let alert = this.alertCtrl.create({
        title: 'Disabled Date Selected!',
        subTitle: 'You have selected dates that are not available. Please select dates that are available.',
        buttons: ['OK']
        });
        alert.present();
    }
    ...
    canDone(): boolean {
        ...
        if(this._d.isRadio){
            return (!!(this.dayTemp[0] && this.dayTemp[0].time)) && !this._disableDateSelected;
        }else {
            return (!!(this.dayTemp[0] && this.dayTemp[1]) && !!( this.dayTemp[0].time && this.dayTemp[1].time)) && !this._disableDateSelected;
        }
    }
